...
#Table of missingness in every variable
sapply(meta, function(x) sum(is.na(x)))


##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
# Create short dataframe for link to run numbers ####
##~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

seq_id <- meta %>% select(id, accession, public_accession, project_accession, project_name)
write.csv(seq_id, "sequence_ids.csv", row.names = F)
